...
func PrintConfig(path string, config *Config) error {
	...
	if config.Username != "" {
		td = append(td, []string{pterm.FgCyan.Sprint("username"), pterm.FgGreen.Sprint(config.Username)})
	}
	if config.Password != "" {
		td = append(td, []string{pterm.FgCyan.Sprint("password"), pterm.FgGreen.Sprint(config.Password)})
	}
